...
using namespace std;
const int maxn=3e6+10;//Èý±¶¿Õ¼ä 
int wa[maxn],wb[maxn],ws[maxn],
	wv[maxn],sa[maxn],r[maxn],
	rank[maxn],height[maxn],n,_p;
bool c0(int*,int,int);
bool c12(int,int*,int,int);
void sort(int*,int*,int*,int,int);
void dc3(int*,int*,int,int);
...
int main()
{
	...
	dc3(r,sa,n+1,128);
	calheight(r,sa,n);
	for(int i=1;i<=n;i++) output(sa[i]+1);
	putchar('\n');
	for(int i=2;i<=n;i++) output(height[i]);
	return 0;
}
